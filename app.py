...
# ✅ Added to suppress SSL warnings when verify=False is used
import urllib3
...
# Ensure schema exists even when running under a WSGI server
try:
    init_db()
...
def scrape_incidents():
    ...
    try:
        ...
        # ✅ First request establishes session cookie (SSL verify disabled)
        response = session.get(base_url, timeout=15, allow_redirects=True, verify=False)

        # ✅ If redirected to cookie check URL, follow it (SSL verify disabled)
        if 'AspxAutoDetectCookieSupport' in response.url or response.status_code == 302:
            response = session.get(
                f"{base_url}?AspxAutoDetectCookieSupport=1",
                timeout=15,
                verify=False
            )
